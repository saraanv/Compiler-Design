...
def p_statement(p):
    ...
    if len(p) == 5 and p[2] == '=':
        ...
        temp = new_temp()
        generate_code("ASSIGN", temp, p[3])
        generate_code("ASSIGN", p[1], temp)
        p[0] = ("assign", p[1], temp)
    elif len(p) == 8:
        label_true = new_label()
        label_false = new_label()
        generate_code("IF", p[3], label_true, label_false)
        p[0] = ("if_else", p[3], p[5], p[7], label_true, label_false)
    elif len(p) == 6 and p[1] == 'if':
        label_true = new_label()
        label_false = new_label()
        generate_code("IF", p[3], label_true, label_false)
        p[0] = ("if", p[3], p[5], label_true, label_false)
    elif len(p) == 3 and p[1] == 'return':
        p[0] = ("return", p[2])
    elif len(p) == 6 and p[1] == 'while':
        label_start = new_label()
        label_end = new_label()
        generate_code("LABEL", label_start)
        generate_code("IF", p[3], label_end, label_start)
        p[0] = ("while", p[3], label_start, label_end)
    else:
        p[0] = None
...
def p_expr(p):
    ...
    if len(p) == 2:
        ...
        if p[1] == '(':
            ...
            temp = new_temp()
            generate_code(p[2], p[1], p[3], temp)
            p[0] = temp
    elif len(p) == 3:
        temp = new_temp()
        generate_code(p[1], p[2], temp)
        p[0] = temp
...
for i, data in enumerate(test_cases):
    ...
    print("Generated Three Address Code:", three_address_code)
